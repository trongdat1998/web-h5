...
class PaymentRC extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      (this.props.orderInfo && !this.props.orderInfo.expired) && nextProps.orderInfo.expired
    ) {
      this.setState({
  ...
  run = () =>{
    if(this.state.remainTime && Math.max(0, this.state.remainTime - 1000) <= 0){
      this.props.dispatch({
  ...
  render() {
    ...
    if(payStatus == ""){
      return <div></div>
    }else if(payStatus == "WAIT_FOR_PAYMENT"){ // 待支付
      return(
        <div className={classes.payment}>
          <h1>{this.props.intl.formatMessage({ id: "请确认以下为本人操作" })}</h1>
          <div className={classes.info}>
            <ul>
              ...
              {this.props.payList && this.props.payList.length ? 
                <li>
                  <span>{this.props.intl.formatMessage({ id: "支付金额" })} ：</span>
                  <p>
                    {this.props.payList.map((item, index)=>{
                      return (
                        <span key={index}>{item.amount} {item.tokenId}</span>
                      )
                    })}
                  </p>
                </li> : ""
              ...
              {this.props.prepayList && this.props.prepayList.length ? 
                <li>
                  <span>{this.props.intl.formatMessage({ id: "预付金额" })} ：</span>
                  <p>
              ...
              {this.props.isMapping ? 
                <li>
                  <span>{this.props.intl.formatMessage({ id: "映射金额" })} ：</span>
                  <p>
              ...
              {need2FA ? 
                <li>
                  <span>{this.props.intl.formatMessage({ id: "验证码" })} ：</span>
            ...
            <Button variant="contained" color="primary" fullWidth={true} onClick={this.pay.bind(this, authType)}>
          ...
          <FormattedHTMLMessage
            id="请在{time}之内完成操作"
            values={{
              time: (remainTime[0] != "00" ? remainTime[0] : "") + 
              (remainTime[0] != "00" ? this.props.intl.formatMessage({ id: "天" }) + " " : "") +
              remainTime[1]+":" + remainTime[2] + ":" + remainTime[3]
            }}
            tagName="p"
          />
        </div>
