...
class TokenInfoRC extends React.Component {
  ...
  countShadow = () => {
    ...
    if (pathname.indexOf(route_map.future_trade) > -1) {
      a.reverse();
    }
    ar.map((it, i) => {
      const item = data.b[i];
  ...
  renderTrade = () => {
    ...
    const quote =
      this.props.symbol_id && symbol_quote[this.props.symbol_id]
        ...
        : {};
    const token = this.props.future_info.baseTokenFutures
      ? this.props.future_info.baseTokenFutures.displayTokenId
      : "";
    const cRates =
      quote.c && token
    ...
    return (
      <Grid container className={classes.handicap_exchange}>
        ...
        <Grid item xs={6}>
          ...
          >
            <Grid item>
              {this.props.future_info.baseTokenFutures
                ? this.props.future_info.baseTokenFutures.displayTokenId
                : ""}
            </Grid>
            <Grid item>
